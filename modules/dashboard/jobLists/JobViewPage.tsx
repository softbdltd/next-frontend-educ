...
const KeyValuePairText = ({
  ...
  return condition ? (
    <Typography className={classes.text_mutate} sx={{margin: '8px 0px'}}>
      <span className={classes.text_bold}>{itemKey}</span>{' '}
      {itemValue ?? messages['common.n_a']}
    </Typography>
  ) : null;
};
...
const JobViewPage = () => {
  ...
  const getEducationalRequirements = () => {
    ...
    /*console.log(
      'conditions',
      'abcd 1:',
      jobData?.candidate_requirements?.degrees,
      'abcd 2:',
      educationalInstitutes,
      'abcd 3:',
      professionalCertificates,
  ...
  const ApplyButton = ({btnText, ...props}: IApplyButton) => {
    return (
      ...
      <Grid container sx={{marginTop: 2}}>
        ...
        <Grid item xs={12}>
          ...
          <Paper className={classes.jobDetailsBox}>
            ...
            {authUser?.isYouthUser && (
              <ApplyButton
                btnText={messages['industry.apply_now'] as string}
                onClick={onJobApply}
              />
            )}
          </Paper>
        ...
        <Box
          ...
          <Grid container>
            ...
            {/*   KeyValuePairTextJob  Secctor*/}
            <Grid item xs={12}>
              {/*<JobPreviewSubComponent title={messages['job.vacancy']}>*/}
              {/*  {jobData?.primary_job_information?.no_of_vacancies*/}
              ...
              <JobPreviewSubComponent
                title={messages['job_posting.job_responsibility'] as string}>
                {getResponsibilities()}
              </JobPreviewSubComponent>

              <JobPreviewSubComponent
                title={
                  messages['job_posting.educational_requirements'] as string
                }>
                {getEducationalRequirements()}
              </JobPreviewSubComponent>
              <JobPreviewSubComponent
                title={
                  messages['job_posting.experience_requirements'] as string
                }>
                {getExperienceRequirements()}
              </JobPreviewSubComponent>
              <JobPreviewSubComponent
                title={
                  messages['job_posting.additional_requirements'] as string
                }>
                {getAdditionalRequirements()}
              </JobPreviewSubComponent>
              {jobData?.additional_job_information?.work_places && (
                <JobPreviewSubComponent
                  title={messages['job_posting.workplace'] as string}>
                  {getWorkplace()}
                </JobPreviewSubComponent>
              )}

              <JobPreviewSubComponent
                title={messages['job_preview_summary.job_location']}>
                <ul>
                  <li>{getLocations()}</li>
                </ul>
              </JobPreviewSubComponent>
              ...
              <JobPreviewSubComponent
                title={
                  messages[
                    'job_posting.compensation_and_other_benefits'
                  ] as string
                }>
                {getOtherBenefits()}
              </JobPreviewSubComponent>
